...
There is one component in the manual, plugins. On the website it is possible to navigate the plugins component. Each component can have a number of modules. ROOT is an important module that contains the nav.adoc page that provides the link with the plugin pages. Nav.adoc is comprized of the partial directory files nav-main.adoc, nav-beta.adoc and nav-all.adoc which are used to construct the left hand navigation bar, which will not be touched in the following edits.
